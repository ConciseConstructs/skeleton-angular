...
export class OfflineService {
  ...
  private synchronizedQuizes:boolean|null
      ...
      private setupEventListeners() {
        try { this._setupEventListeners() }
          ...
          private _setupEventListeners() {
            ...
            this.events.onRequestTagsSyncOutcome.subscribe(outcome => this.onRequestTagsSyncOutcome(outcome))
            this.events.onRequestQuestionsSyncOutcome.subscribe(outcome => this.onRequestQuestionsSyncOutcome(outcome))
            this.events.onRequestQuizesSyncOutcome.subscribe(outcome => this.onRequestQuizesSyncOutcome(outcome))
  ...
  public createDbIfNotExist() {
    try {
      ...
      let quizes = localStorage.getItem(`${ this.session.acctId }.quizes`)
      if (!quizes) localStorage.setItem(`${ this.session.acctId }.quizes`, JSON.stringify([]))
  ...
  public insertItem(params:{ table:string, record:any }):any[] {
    try { return this._insertItem(params) }
      ...
      this.events.onError.next({ signature: '542b366e-09f3-40d2-b6d3-681d832e7631', details: error })
    }
  }
  ...
  public updateItem(params:{ table:string, record:any }):any[] {
    try { return this._updateItem(params) }
      ...
      this.events.onError.next({ signature: '201b879a-3102-415c-afd4-06a10972c777', details: error })
    }
  }
  ...
  public deleteItem(params:{ table:string, id:string }):any[] {
    try { return this._deleteItem(params) }
      ...
      this.events.onError.next({ signature: '9f5b2bb0-92a3-4375-bb7f-d1972871eb66', details: error })
    }
  }
  ...
  public syncOfflineRecords() {
    try { this._syncOfflineRecords() }
      ...
      this.events.onError.next({ signature: '6def7bf4-3e98-49fe-bd75-34f411704f81', details: error })
    }
  }
      ...
      private _syncOfflineRecords() {
        ...
        this.syncRemovals()
          ...
          private syncQuizes() {
            let quizes = (JSON.parse(localStorage.getItem(`${ this.session.acctId }.quizes`)) as Quiz[]).filter(q => q.offline === true)
            if (quizes.length > 0) this.events.onRequestQuizesSync.next(quizes)
            else this.synchronizedQuizes = true
            this.monitorRecordsSyncCompletion()
          }
  ...
  private onRequestQuizesSyncOutcome(outcome) {
    try { this._onRequestQuizesSyncOutcome(outcome) }
    catch (error) {
      ...
      private _onRequestQuizesSyncOutcome(outcome) {
        if (outcome.success) this.synchronizedQuizes = true
        else this.synchronizedQuizes = false
        this.monitorRecordsSyncCompletion()
      }
  ...
  private monitorRecordsSyncCompletion() {
    if (this.synchronizedTags === null || this.synchronizedQuestions === null || this.synchronizedQuizes === null || this.synchronizedRemovals === null) return
    else this.events.onSynchronizedDatabase.next({ success: true, details: { tags: this.synchronizedTags, questions: this.synchronizedQuestions, quizes: this.synchronizedQuizes, removals: this.synchronizedRemovals }})
  }
